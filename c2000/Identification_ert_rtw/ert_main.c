...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(Identification_M, 0);
  ...
  errorCode = extmodeInit(Identification_M->extModeInfo, &rtmGetTFinal
    (Identification_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Identification_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Identification_M);
  enableTimer0Interrupt();
  globalInterruptEnable();
  while (runModel) {
